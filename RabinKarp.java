class RabinKarpSearch  
{
	...
	public int patternSearch(char []text,char []patt)
	{
		int m=patt.length;
		int n=text.length;
		...
		{
			if(pattHash==textHash && checkEqual(text,i-1,i+m-2,patt,0,m-1)){
				return i-1;
			}
			if(i<n-m+1)
			{
	...
	private long recalculateHash(char []str,int oldInd,int newindex,long oldHash,int pattLen)
	{
		long newHash=oldHash-str[oldInd];
		newHash/=prime;
		newHash+=str[newindex]*Math.pow(prime,pattLen - 1);
		return newHash;
	}
